...
# TODO JULIO: maybe add functionality so that only caltech.edu links are fetched
...
def fetch_html_page(url):
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=2) as usock:
            # DEBUG
            # print(f"usock type: {type(usock.info())}")
            # print(f"SUCL: {usock.info()}")
            # assert False
            ...
            # else:  # DEBUG
                # print(f"HREE MF: {usock.info()['content-type']}")
    # Terminate on CTRL+C sequences, and pass URLError up the stack.
    except KeyboardInterrupt:
        raise
    # except URLError as ue:  # DEBUG
        # print(f"ETF: {ue}")
    except:
        pass
...
#     else:
#         sys.stdout.write(','.join(fetch_links(sys.argv[1])))
